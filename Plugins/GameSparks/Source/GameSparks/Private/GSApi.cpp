...
void XboxOneBuyGoodsRequestResponseCallback(GameSparks::Core::GS& gsInstance, const GameSparks::Api::Responses::BuyVirtualGoodResponse& response){
    ...
    UGSXboxOneBuyGoodsRequest* g_UGSXboxOneBuyGoodsRequest = static_cast<UGSXboxOneBuyGoodsRequest*>(response.GetUserData());
                                             
    if (response.GetHasErrors())
    {
        g_UGSXboxOneBuyGoodsRequest->OnResponse.Broadcast(unreal_response, true);
    }
    else
    {
        g_UGSXboxOneBuyGoodsRequest->OnResponse.Broadcast(unreal_response, false);
    }
}

UGSXboxOneBuyGoodsRequest* UGSXboxOneBuyGoodsRequest::SendXboxOneBuyGoodsRequest(FString CurrencyCode, FString ItemId, int32 ItemsConsumed, float SubUnitPrice, bool UniqueTransactionByPlayer,  UGameSparksScriptData* ScriptData, bool Durable, int32 RequestTimeoutSeconds)
{
	UGSXboxOneBuyGoodsRequest* proxy = NewObject<UGSXboxOneBuyGoodsRequest>();
	proxy->currencyCode = CurrencyCode;
	proxy->itemId = ItemId;
	proxy->itemsConsumed = ItemsConsumed;
	proxy->subUnitPrice = SubUnitPrice;
	proxy->uniqueTransactionByPlayer = UniqueTransactionByPlayer;
	proxy->scriptData = ScriptData;
	proxy->durable = Durable;
...
void UGSXboxOneBuyGoodsRequest::Activate()
{
	GameSparks::Api::Requests::XboxOneBuyGoodsRequest gsRequest(UGameSparksModule::GetModulePtr()->GetGSInstance());
	if(currencyCode != ""){
		gsRequest.SetCurrencyCode(TCHAR_TO_UTF8(*currencyCode));
	}
	if(itemId != ""){
		gsRequest.SetItemId(TCHAR_TO_UTF8(*itemId));
	}
	if(itemsConsumed != 0){
		gsRequest.SetItemsConsumed(itemsConsumed);
	}
	if(subUnitPrice != 0.0f){
		gsRequest.SetSubUnitPrice(subUnitPrice);
	}
    ...
    if(requestTimeoutSeconds > 0){
    	gsRequest.Send(XboxOneBuyGoodsRequestResponseCallback, requestTimeoutSeconds);	
    } else {
    	gsRequest.Send(XboxOneBuyGoodsRequestResponseCallback);
    }
...
UGSXboxOneBuyGoodsRequest::~UGSXboxOneBuyGoodsRequest()
{
 if (UGameSparksModule* module = UGameSparksModule::GetModulePtr())
 {
  if (module->IsInitialized())
