...
{
	GS_CODE_TIMING_ASSERT();
	if (m_GSPlatform->GetDontRetryAfterDisconnect()) {
		if (m_Connections.size() > 0 && m_Connections[0]->m_Stopped) {
			if (GameSparksNotConnected && !m_NotConnectedFired) {
				m_NotConnectedFired = true;

				GameSparksNotConnected(*this);
			}
		}
	} else {
		ConnectIfRequired();
